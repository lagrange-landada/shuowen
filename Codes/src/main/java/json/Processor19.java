...
/***
 * Created by zhengyu.shang on 2024/11/19.   ---用来导出《释名》SQL
 */

public class Processor19 {

    private static final String SOURCE = "釋名";
            ...
            if (paragraphs != null && paragraphs.size() > 0) {
                for (XWPFParagraph paragraph : paragraphs) {
                    ...
                    if (paragraphText.length() == 0) {
                        continue;
                    } else if (paragraphText.matches("釋名")) {
                        continue;
                        ...
                        Set<String> flagList = new HashSet<>();
                            ...
                            for (String ss : run.text().codePoints()
                                ...
                                if (Objects.equals(color, "4C90CA")) {
                                    if (Objects.equals(color, beforeColor)) {
                                        ...
                                        boolean change = flagList.add((num) + "");
                                        wordReals.add((change ? ";" : "/") + ss);
                                    }
                                    beforeWord = "wordReal";
                            }
                            // 绿色
                                if (Objects.equals(color, "008000")) {
                                    switch (beforeWord) {
                                        case "word":
                                            words.set(words.size() - 1, words.get(words.size() - 1) + (Objects.equals(color, beforeColor) ? "" : "/") + ss);
                                            break;
                                        case "wordReal":
                                            wordReals.set(wordReals.size() - 1, wordReals.get(wordReals.size() - 1) + (Objects.equals(color, beforeColor) ? "" : "/") + ss);
                                            break;
                                    }
                                }
                                // 绿色
